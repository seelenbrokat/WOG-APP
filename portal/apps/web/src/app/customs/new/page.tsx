...
function NewShipmentInner() {
  ...
  function toggleExtra(code: keyof ShipmentExtras, checked: boolean) {
    // Verzollungsauftrag: Verzollung bleibt immer aktiv
    if (code === 'verzollung' && !checked) return;
    setExtras((prev) => {
      const next = { ...prev, [code]: checked, verzollung: true };
  ...
  useEffect(() => {
    ...
    api<any[]>('/mandanten').then((m) => {
      setMandanten(m);
      const customsMandant =
        m.find((x) => String(x.code || '').toUpperCase() === 'GMBH') ||
        m.find((x) => String(x.code || '') === '2') ||
        m.find((x) => /gmbh|\b2\b/i.test(String(x.name || ''))) ||
        (m.length >= 2 ? m[1] : m[0]);
      setForm((f) => ({
        ...f,
        mandantId: presetMandantId || f.mandantId || customsMandant?.id || m[0]?.id || '',
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const extrasPayload: ShipmentExtras = {
        ...extras,
        verzollung: true,
        begleitpapiere: extras.begleitpapiere !== false,
      };
  ...
  return (
    <AppShell title="Verzollungsauftrag" eyebrow="Verzollung">
      <form className="panel stack" onSubmit={onSubmit}>
        <div className="row" style={{ justifyContent: 'space-between' }}>
          <p className="muted" style={{ margin: 0 }}>
            Verzollungsauftrag mit Belegen – Mandant 2 (GmbH) und Verzollung sind vorausgewählt.
            Zustellland standardmäßig Schweiz.
          </p>
          <Link href="/addresses">Adressbuch verwalten</Link>
        </div>

        <div className="tabs" role="tablist" aria-label="Verzollungsauftrag">
          {TABS.map((t) => (
            <button
              key={t.id}
              type="button"
                ...
                <div className="extras-grid">
                  {SHIPMENT_EXTRA_OPTIONS.filter((o) => o.group === group).map((opt) => (
                    <label key={opt.code} className="row">
                      <input
                        type="checkbox"
                        checked={opt.code === 'verzollung' ? true : Boolean(extras[opt.code])}
                        disabled={opt.code === 'verzollung'}
                        onChange={(e) => toggleExtra(opt.code, e.target.checked)}
                      />
                      <span>
                        {opt.label}
                        {opt.code === 'verzollung' ? ' (Pflicht)' : ''}
                      </span>
                    </label>
                  ))}
                </div>
        ...
        <div className="shipment-form-actions row" style={{ justifyContent: 'space-between' }}>
          ...
          <button className="btn btn-primary" type="submit">
            Verzollungsauftrag übermitteln
          </button>
        </div>
      </form>
    </AppShell>
...
export default function NewCustomsOrderPage() {
  return (
    <Suspense fallback={<AppShell title="Verzollungsauftrag" eyebrow="Verzollung">Laden…</AppShell>}>
      <NewShipmentInner />
    </Suspense>
  );
}
